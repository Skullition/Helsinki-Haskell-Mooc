...
allEqual xs = length xs == length (takeWhile (head xs ==) xs)
...
longest :: Ord a => [[a]] -> [a]
longest (el : []) = el
longest (first : second : list)
  | length first > length second = longest (first : list)
  | length first < length second = longest (second : list)
  | otherwise =
    if first < second
      then longest (first : list)
      else longest (second : list)
...
incrementKey _ [] = []
incrementKey key (el : list) =
  if fst el == key
    then (fst el, snd el + 1) : incrementKey key list
    else (fst el, snd el) : incrementKey key list
...
transfer from to amount bank =
  if Map.notMember from bank || Map.notMember to bank || amount < 0 || transfer' from amount bank
    then bank
    else reduce from amount $ add to amount bank
  where
    transfer' account amount bank =
      case Map.lookup account bank of
        Nothing -> True
        Just value -> if value < amount then True else False
    reduce account amount bank =
      case Map.lookup account bank of
        Nothing -> bank
        Just value -> Map.insert account (value - amount) bank
    add account amount bank =
      case Map.lookup account bank of
        Nothing -> bank
        Just value -> Map.insert account (value + amount) bank
...
maxIndex arr = fst (head (sortBy (\x y -> compare (snd y) (snd x)) (assocs arr)))
