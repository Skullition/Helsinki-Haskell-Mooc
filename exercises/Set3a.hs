...
firstHalf xs
  ...
  | otherwise = take (div (length xs) 2 + 1) xs

-- firstHalf xs = firstHalf' (div (length xs) 2) xs

-- firstHalf' :: String -> String
-- firstHalf' 0 _ = []
-- firstHalf' n (x : xs) = (n -1) (x : firstHalf' xs)
...
joinToLength n string = [result | first <- string, second <- string, let result = first ++ second, length result == n]
...
sumRights value = sumEither $ rights value
  where
    sumEither [] = 0
    sumEither (x : xs) = x + sumEither xs
...
multiCompose fs value = multiCompose' (reverse fs) value
  where
    multiCompose' [] value = value
    multiCompose' (f : fs) value = multiCompose' fs $ f value
