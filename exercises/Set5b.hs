...
treeMax Empty = 0
treeMax node = maximum (treeMax' node)

treeMax' :: Tree Int -> [Int]
treeMax' Empty = []
treeMax' (Node val a b) = val : treeMax' a ++ treeMax' b
...
allValues condition (Node val b c) = condition val && allValues condition b && allValues condition c
...
mapTree f (Node val b c) = Node (f val) (mapTree f b) (mapTree f c)
...
cull _ Empty = Empty
cull val (Node root a b) = if val == root then Empty else Node root (cull val a) (cull val b)
...
isOrdered (Node val a b) = allValues (\a -> a < val) a && allValues (\a -> a > val) b && isOrdered a && isOrdered b
...
walk (StepL : steps) (Node _ a _) = walk steps a
walk (StepR : steps) (Node _ _ b) = walk steps b
...
set _ _ Empty = Empty
set [] val (Node nodeValue a b) = Node val a b
set (StepL : path) val (Node nodeValue a b) = Node nodeValue (set path val a) b
set (StepR : path) val (Node nodeValue a b) = Node nodeValue a (set path val b)
...
search val (Node nodeValue left right) =
  if val == nodeValue
    then Just []
