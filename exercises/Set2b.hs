...
oddFactorial 1 = 1
oddFactorial input
  | even input = oddFactorial (input - 1)
  | otherwise = input * oddFactorial (input - 1)
...
leftpad string howLong
  | length string >= howLong = string
  | otherwise = leftpad' string (howLong - length string)

leftpad' :: String -> Int -> String
leftpad' string 0 = string
leftpad' string howLong = " " ++ leftpad' string (howLong - 1)
