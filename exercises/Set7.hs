...
member element (Set []) = False
member element (Set (x : xs))
  | element == x = True
  | otherwise = member element (Set xs)

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add ele (Set xs) = if elem ele xs then Set xs else Set $ add' ele xs
  where
    add' ele xs = sort $ ele : xs
...
data State = Start | Error | Finished | Add | Dough | Sweet | Batter | Mixed
  deriving (Eq, Show)
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (def :| xs) = head result :| tail result
  where
    result = reverse (def : xs)
...
passwordAllowed password (MinimumLength len) = len <= length password
passwordAllowed [] (ContainsSome con) = False
passwordAllowed (x : password) (ContainsSome con) = elem x con || passwordAllowed password (ContainsSome con)
passwordAllowed password (DoesNotContain con) = not $ passwordAllowed password (ContainsSome con)
...
data Arithmetic = Value Integer | Plus Arithmetic Arithmetic | Multiply Arithmetic Arithmetic
  deriving (Show)

literal :: Integer -> Arithmetic
literal int = Value int

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation "+" a b = Plus a b
operation "*" a b = Multiply a b
operation _ _ _ = Value 0
